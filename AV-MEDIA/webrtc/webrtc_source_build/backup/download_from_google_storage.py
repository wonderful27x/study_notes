...
class Gsutil(object):
  ...
  def check_call(self, *args):
    cmd = [self.VPYTHON3, self.path]
    cmd.extend(args)
    print('gsutil.check_call,cmd: ' + str(cmd))
...
def _downloader_worker_thread(thread_num, q, force, base_url,
  ...
  while True:
    ...
    print('worker_thread running while true...')
    print('queue.get(),input_sha1_sum=' + str(input_sha1_sum) + ',output_filename=' + str(output_filename))
    ...
    print('jump for checking file exists, file_url: ' + file_url)
    code = 0
    if code != 0:
      if code == 404:
    ...
    url = 'https://storage.googleapis.com/' + file_url[5:]
    print('use curl to download, url: ' + url)
    subprocess2.check_call(['curl','-x','socks5h://127.0.0.1:1080','-L', url, '-o', output_filename])
    code = 0
    if code != 0:
      ...
      with tarfile.open(output_filename, 'r:gz') as tar:
        ...
        out_q.put('%d> Extracting %d entries from %s to %s' %
                  (thread_num, len(tar.getmembers()),output_filename,
                   extract_dir))
        print('open: ' + extract_dir + '.tmp' + ',then tar.extractall...')
        with open(extract_dir + '.tmp', 'a'):
          tar.extractall(path=dirname)
        os.remove(extract_dir + '.tmp')
    print('set executable bit...')
    # Set executable bit.
    if sys.platform == 'cygwin':
      print('sys.platform=cygwin')
      ...
      stat_file = st + '.stat'
      url = 'https://storage.googleapis.com/' + file_url[5:]
      cmd = ['curl','-x','socks5h://127.0.0.1:1080','-I', url,'-o',stat_file]
...
def download_from_google_storage(
  ...
  print('running download_from_google_storage(...)...')
  ...
  for sha1, path in input_data:
    print('work_queue.put(sha1=' + str(sha1) + ',path=' + str(path) + ')')
    work_queue.put((sha1, path))
  for _ in all_threads:
    print('work_queue.put(None,None),to tell threads to stop')
