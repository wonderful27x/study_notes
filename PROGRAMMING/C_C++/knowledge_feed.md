...
# protected 的析构函数
* **不想让用户直接构造一个类的对象，而希望构造这个类的子类，通常接口都这样使用**
```c++
class MyClass {
    template<typename T> friend class std::default_delete;
public:
...
// 正确可以创建堆对象，但是调用delete是错误的
MyClass *mcls = new ClassB;
delete mcls;
// 正确可以创建堆对象，但是调用delete是错误的
...
MyClass mycls;
// 正确且可以析构
std::shared_ptr<MyClass> cls1(new ClassB);
//只有声明了友元：template<typename T> friend class std::default_delete;才是正确的
std::unique_ptr<MyClass> cls3(new ClassB);
// 正确
std::unique_ptr<MyClass> cls4(new MyClass);
// 错误
std::shared_ptr<MyClass> cls2(new MyClass); 
